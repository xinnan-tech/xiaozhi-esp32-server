...
class Live2DManager {
    ...
    async initializeLive2D() {
        try {
            ...
            // 清理之前的 PIXI 应用（防止 WebGL 上下文累积）
            if (this.live2dApp) {
                try {
                    this.live2dApp.destroy(true, { children: true, texture: true });
                } catch (e) { /* 忽略清理异常 */ }
                this.live2dApp = null;
                this.live2dModel = null;
            }
            ...
            // 使用 canvas 容器尺寸（适配 max-width:480 的 H5 容器）
            const canvasContainer = canvas.parentElement;
            const containerW = canvasContainer ? canvasContainer.clientWidth : window.innerWidth;
            const containerH = canvasContainer ? canvasContainer.clientHeight : window.innerHeight;

            // 限制 resolution 最大为 2，避免高分屏 WebGL 纹理溢出
            ...
            const fitModelToCanvas = () => {
                ...
                const h = canvasContainer ? canvasContainer.clientHeight : window.innerHeight;
                this.live2dApp.renderer.resize(w, h);
                const baseScale = Math.min(w / Math.max(this.live2dModel.width, 1), h / Math.max(this.live2dModel.height, 1));
                const scale = Math.max(0.38, Math.min(baseScale * 2.35, 1.25));
                this.live2dModel.scale.set(scale);
                this.live2dModel.x = (w - this.live2dModel.width) * 0.5;
                this.live2dModel.y = Math.max(-42, (h - this.live2dModel.height) * 0.16);
            };
            fitModelToCanvas();
            ...
            // 添加窗口大小变化监听器，保持模型在当前 Canvas 中间显示
            this.resize = fitModelToCanvas;
    ...
    initializeAudioAnalyzer() {
        try {
            ...
            if (!audioPlayer) {
                console.debug('音频播放器未初始化，使用模拟口型动画');
                return false;
            }
    ...
    animateMouth() {
        ...
        if (internal && internal.coreModel) {
            ...
            if (this.analyser && this.dataArray) {
                ...
                // 调试日志：每60帧输出一次（避免控制台刷屏）
                if (!this._debugFrameCount) this._debugFrameCount = 0;
                this._debugFrameCount++;
                if (this._debugFrameCount % 60 === 1) {
                    console.log(`[Live2D] 模型: ${this.currentModelName || 'unknown'}, 音量: ${average?.toFixed(0)}, OpenY: ${mouthOpenY.toFixed(3)}`);
                }
            }

            if (!this.analyser || !this.dataArray) {
                average = 80 + Math.sin(performance.now() / 90) * 48 + Math.sin(performance.now() / 37) * 18;
                mouthOpenY = 0.18 + (Math.max(0, Math.min(1, average / 145)) * 0.72);
            }
    ...
    startTalking() {
        ...
        if (!this.analyser) {
            if (!this.initializeAudioAnalyzer()) {
                console.debug('音频分析器初始化失败，使用模拟口型动画');
    ...
    destroy() {
        ...
        if (this.live2dModel) {
            try {
                this.live2dModel.interactive = false;
                this.live2dModel.removeAllListeners();
                if (this.live2dApp && this.live2dApp.stage) {
                    this.live2dApp.stage.removeChild(this.live2dModel);
                }
                if (typeof this.live2dModel.destroy === 'function') {
        ...
        // 清理 PIXI 应用（释放 WebGL 上下文）
        if (this.live2dApp) {
            try {
                this.live2dApp.destroy(true, { children: true, texture: true, baseTexture: true });
            } catch (e) { /* 忽略清理异常 */ }
            this.live2dApp = null;
        }
    }
