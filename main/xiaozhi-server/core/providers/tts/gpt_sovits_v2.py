...
class TTSProvider(TTSProviderBase):
    ...
    def __init__(self, config, delete_audio_file):
        ...
        self.ref_audio_path = config.get('ref_audio') if config.get(
            'ref_audio') else config.get("ref_audio_path")

        self.prompt_text = config.get('ref_text') if config.get(
            'ref_text') else config.get("prompt_text")
        ...
        self.batch_threshold = float(
            batch_threshold) if batch_threshold else 0.75
