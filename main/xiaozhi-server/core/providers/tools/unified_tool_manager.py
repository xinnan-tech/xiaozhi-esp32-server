...
class ToolManager:
    ...
    def get_function_descriptions(self) -> List[Dict[str, Any]]:
        """Get all tool function descriptions (OpenAI format)"""
    ...
    async def execute_tool(
        ...
        try:
            ...
            if not executor:
                return ActionResponse(
                    action=Action.ERROR,
                    response=f"Executor for tool type {tool_type.value} not registered",
                )

            # 执行工具
            self.logger.info(f"Execute tool: {tool_name}, args: {arguments}")
    ...
    def get_tool_statistics(self) -> Dict[str, int]:
        ...
        for tool_type, executor in self.executors.items():
            try:
                ...
                self.logger.error(f"Error getting {tool_type.value} tool stats: {e}")
