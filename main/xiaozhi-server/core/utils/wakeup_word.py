        ...
        while True:
            try:
                ...
                if time.time() - self.start_time > self.timeout:
                    raise TimeoutError("Acquiring file lock timeout")
                time.sleep(0.1)
...
class WakeupWordsConfig:
    def __init__(self):
        ...
        self._cache_ttl = 1  # Cache time to live (seconds)
    ...
    def generate_file_path(self, voice: str) -> str:
        """Generate audio file path, using hex digest of voice as filename"""
        try:
            # Generate MD5 hash of voice
            voice_hash = hashlib.md5(voice.encode()).hexdigest()
