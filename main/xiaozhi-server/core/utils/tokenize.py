...
def split_words(
    ...
    *,
    ...
    translation_table = (
        str.maketrans("", "", "".join(PUNCTUATIONS)) if ignore_punctuation else None
    )
...
if __name__ == "__main__":
    ...
    # Chinese (not split by character)
    print(split_words("你好世界"))
    # [('你好世界', 0, 4)]

    # Chinese (split by character)
    print(split_words("你好世界", split_character=True))
    # [('你', 0, 1), ('好', 1, 2), ('世', 2, 3), ('界', 3, 4)]

    # Mixed
    print(split_words("Hello 你好", split_character=True))
    # [('Hello', 0, 5), ('你', 6, 7), ('好', 7, 8)]

    # Only get word list
    words_only = [w[0] for w in split_words("Hello 你好", split_character=True)]
    print(words_only)  # ['Hello', '你', '好']
    print(len(words_only))  # 3