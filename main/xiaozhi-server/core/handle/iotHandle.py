    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if not hasattr(conn, 'loop'):
                ...
                return ActionResponse(Action.ERROR, "Connection对象没有loop属性", "执行操作时出错: Connection对象没有loop属性")
...
def create_iot_function(device_name, method_name, method_info):
    ...
    async def iot_control_function(conn, response_success=None, response_failure=None, **params):
        try:
            # 打印响应参数
            logger.bind(tag=TAG).info(f"控制函数接收到的响应参数: success='{response_success}', failure='{response_failure}'")
...
def create_iot_query_function(device_name, prop_name, prop_info):
    ...
    async def iot_query_function(conn, response_success=None, response_failure=None):
        try:
            # 打印响应参数
            logger.bind(tag=TAG).info(f"查询函数接收到的响应参数: success='{response_success}', failure='{response_failure}'")
...
async def handleIotDescriptors(conn, descriptors):
    ...
    await set_iot_status(conn, "Speaker", "volume", default_iot_volume)
