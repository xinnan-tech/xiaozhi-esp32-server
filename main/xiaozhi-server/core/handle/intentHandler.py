...
async def handle_user_intent(conn, text):
    # Preprocess input text, handle possible json formats
    try:
    ...
    # Use chat methods that support function calling and no longer perform intent analysis.        return False
    # Using llm for intent analysis
    intent_result = await analyze_intent_with_llm(conn, text)
    if not intent_result:
        return False
    # Sentence id is generated at the beginning of the session
...
async def process_intent_result(conn, intent_result, original_text):
    ...
    try:
        ...
        if "function_call" in intent_data:
            # The function call is obtained directly from the intent recognition
            conn.logger.bind(tag=TAG).debug(
                f"Intent result in function call format detected: {intent_data['function_call']['name']}"
            )
            ...
            if function_name == "result_for_context":
                ...
                def process_context_result():
                    ...
                    context_prompt = f"""Current time:{current_time}
                                        ...
                                        Please answer the user's questions based on the above information:{original_text}"""
