    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                "location": {
                    "type": "string",
                    "description": "地点名，例如杭州。可选参数，如果不提供则通过IP定位，如果没有指定特定的地点，例如街道，城市，乡村等；那就直接返回空"
                },
                "keyword": {
                    "type": "string",
...
class PlaceSearchService:
    ...
    def get_location_by_ip(self, client_ip):
        ...
        try:
            ...
            logger.bind(tag=TAG).error(f"高德API获取IP信息: {data}")
            ...
            if data.get("status") == "1":
                result = {
                    ...
                    "rectangle": data.get("rectangle", ""),  # 城市边界矩形，格式为"lng_min,lat_min;lng_max,lat_max"
                    "ip": client_ip
                }
                
                # 如果有rectangle信息，提取中心点作为经纬度
                if result.get("rectangle"):
                    try:
                        rect_parts = result["rectangle"].split(";")
                        if len(rect_parts) == 2:
                            # 取两个点的平均值作为中心点
                            min_lng, min_lat = map(float, rect_parts[0].split(","))
                            max_lng, max_lat = map(float, rect_parts[1].split(","))
                            
                            result["longitude"] = (min_lng + max_lng) / 2
                            result["latitude"] = (min_lat + max_lat) / 2
                            result["location"] = f"{result['longitude']},{result['latitude']}"
                    except Exception as e:
                        logger.bind(tag=TAG).error(f"解析rectangle失败: {e}")
                
                return result
            
            # 如果高德API失败，尝试备用方法
            backup_url = f"https://whois.pconline.com.cn/ipJson.jsp?json=true&ip={client_ip}"
            response = requests.get(backup_url)
            
            if response.status_code == 200:
                backup_data = response.json()
                logger.bind(tag=TAG).error(f"备用API获取IP信息: {backup_data}")
                
                return {
                    "city": backup_data.get("city", ""),
                    "province": backup_data.get("pro", ""),
                    "ip": client_ip
                }
        except Exception as e:
    ...
    def search_places_by_keyword(self, lng, lat, keyword, radius=1000, page=1, page_size=5):
        """
        根据经纬度和关键词搜索附近场所
...
try:
    ...
    def get_place_info(conn, location: str = None, keyword: str = "餐厅", radius: int = 1000, lang: str = "zh_CN"):
        ...
        if location and is_vague_location(location):
            logger.bind(tag=TAG).error(f"获取位置: {location}")
            location = None
        ...
        # 2. 如果没有提供location，尝试通过IP地址定位
            ...
            logger.bind(tag=TAG).error(f"获取IP: {ip_info}")
            
            # 如果IP地址能够解析出经纬度，直接使用经纬度搜索
            if ip_info and "location" in ip_info:
                lng, lat = ip_info["location"].split(",")
                city_name = ip_info.get("city") if ip_info and "city" in ip_info else default_city
                location_desc = city_name
                
                # 使用经纬度搜索附近场所
                places = place_service.search_places_by_keyword(lng, lat, keyword, radius)
                logger.bind(tag=TAG).error(f"使用IP经纬度搜索: {lng},{lat}")
            else:
                # 如果没有经纬度，则使用城市名搜索
                city_name = ip_info.get("city") if ip_info and "city" in ip_info else default_city
                ...
                location_desc = city_name
                logger.bind(tag=TAG).error(f"使用城市名搜索: {city_name}")
