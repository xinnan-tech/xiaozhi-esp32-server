...
async def handle_music_command(conn, text, song_name=None, artist=None, use_api=False):
    ...
    # 如果指定了歌手或明确使用API，则使用在线API播放
    if artist or use_api:
        conn.logger.bind(tag=TAG).info(f"使用在线API播放音乐: {song_name} - {artist if artist else '未指定歌手'}")
    ...
    if song_name != "random" and song_name != "":
        conn.logger.bind(tag=TAG).info(f"本地未找到匹配歌曲，尝试使用在线API: {song_name}")
...
async def play_api_music(conn, song_name, artist=None):
    ...
    try:
        # 1. 获取音乐信息
        music_info = await get_music_info(conn, song_name, artist)
        if not music_info:
        ...
        # 下载音频文件 - 注意传入conn参数
        success = await download_file(conn, audio_url, audio_path)
        if not success:
        ...
        conn.logger.bind(tag=TAG).error(f"API音乐播放出错: {str(e)}")
        conn.logger.bind(tag=TAG).error(traceback.format_exc())
        await send_stt_message(conn, f"播放音乐时出错: {str(e)}")
        return False


async def get_music_info(conn, song_name, artist=None):
    """获取音乐信息（音频URL）"""
    global MUSIC_CACHE
    ...
    try:
        ...
        # 使用异步方式获取数据
        response = await asyncio.to_thread(requests.get, url_builder)
        if response.status_code != 200:
            conn.logger.bind(tag=TAG).error(f"获取音乐信息失败，响应码: {response.status_code}")
            return None
        ...
        if audio_url and not audio_url.startswith("http"):
            ...
            conn.logger.bind(tag=TAG).error("API响应中缺少音频URL信息")
            return None
        ...
        conn.logger.bind(tag=TAG).error(f"获取音乐信息时发生错误: {str(e)}")
        return None


async def download_file(conn, file_url, output_path):
    """下载文件到指定路径"""
    try:
        # 使用异步方式下载文件
        response = await asyncio.to_thread(requests.get, file_url, stream=True)
        if response.status_code != 200:
            conn.logger.bind(tag=TAG).error(f"下载文件失败，响应码: {response.status_code}")
            return False
        ...
        conn.logger.bind(tag=TAG).error(f"下载文件时发生错误: {str(e)}")
        return False