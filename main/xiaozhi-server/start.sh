...
init_env_file() {
    ...
    # For local region, skip public IP detection
    if [ "$REGION" = "local" ] || [ "$REGION" = "LOCAL" ]; then
        echo -e "${BLUE}→ Local region detected, using localhost${NC}"
        public_ip="localhost"
    else
        echo -e "${BLUE}→ Detecting public IP for ${REGION_NAME}...${NC}"
    ...
    # Append new config
    echo "" >> "$ENV_FILE"
    echo "# Auto-generated by start.sh at $(date '+%Y-%m-%d %H:%M:%S') for region: ${REGION_NAME}" >> "$ENV_FILE"
