...
从 data/.config.yaml 拿配置，调真实 ASR provider。
...
参考项目自带的 performance_tester_asr.py 模式：
- 从 config/assets/ 读 >300KB 的 wav 文件当测试音频
- speech_to_text_wrapper 真发请求，10s 超时
- placeholder 检测（access_token / api_key / token）
...
def test_asr_speech_to_text_wrapper_returns_text() -> None:
    ...
    if isinstance(text, dict):
        # FunASR 风格：取 content 字段
        assert text.get("content"), f"ASR dict 结果缺少 content：{text!r}"
    elif isinstance(text, str):
        # 简单 str 风格：非空更稳（不强求，避免静音音频误判）
        pass  # str 可以为空（识别不出），不算失败
