  ...
  <el-drawer :visible.sync="dialogVisible" direction="rtl" size="80%" :wrapperClosable="false" :withHeader="false">
    ...
    <div class="function-manager">
      ...
      <div class="params-column">
        ...
        <div v-else class="empty-tip">Please select configured functions to set parameters</div>
      </div>
    </div>
          ...
          <div class="status-container">
            ...
            <span class="status-text">{{
              mcpStatus === 'connected' ? 'Connected' :
                mcpStatus === 'loading' ? 'Loading...' : 'Not Connected'
            }}</span>
  ...
  methods: {
    ...
    flushJson(field) {
      ...
      try {
        ...
        this.$message.error(`${this.currentFunction.name} ${key} field format error: Invalid JSON format`);
      }
    },
