...
class SemanticMusicSearch:
    ...
    def search(self, query: str, limit: Optional[int] = None) -> List[MusicSearchResult]:
        ...
        try:
            ...
            search_results = self.qdrant_client.query_points(
                ...
                with_payload=True,  # Explicitly include payload
                with_vectors=False,  # Don't include vectors in response
                score_threshold=self.min_score_threshold  # Apply threshold at query level
            )
            
            # Convert to MusicSearchResult objects
            results = []
            for hit in search_results.points:  # Note: results are in .points attribute
    ...
    def search_with_filters(self, query: str, language_filter: Optional[str] = None, 
                           keyword_filter: Optional[str] = None, limit: Optional[int] = None) -> List[MusicSearchResult]:
        """
        ...
        try:
            ...
            if keyword_filter:
                filter_conditions.append(
                    FieldCondition(key="keywords", match=Match(any=[keyword_filter]))
                )
            
            query_filter = Filter(must=filter_conditions) if filter_conditions else None
            
            # Search in Qdrant using the new query_points method with filters
            search_results = self.qdrant_client.query_points(
                collection_name=self.collection_name,
            ...
            self.logger.debug(f"Found {len(results)} matching songs with filters for query: '{query}'")
            return results
    ...
    def get_similar_songs(self, song_title: str, language: str, limit: int = 5) -> List[MusicSearchResult]:
        """Find songs similar to a given song"""
        # First find the song to get its embedding
        filter_conditions = [
            FieldCondition(key="title", match=Match(value=song_title)),
            FieldCondition(key="language", match=Match(value=language))
        ]
        
        try:
            # Get the target song
            target_results = self.qdrant_client.query_points(
                collection_name=self.collection_name,
                query=Filter(must=filter_conditions),
                limit=1,
                with_payload=True,
                with_vectors=True
            )
            
            if not target_results.points:
                return []
            
            target_vector = target_results.points[0].vector
            
            # Search for similar songs
            similar_results = self.qdrant_client.query_points(
                collection_name=self.collection_name,
                query=target_vector,
                limit=limit + 1,  # +1 to exclude the original song
                with_payload=True,
                with_vectors=False
            )
            
            # Convert to MusicSearchResult objects and exclude the original song
            results = []
            for hit in similar_results.points:
                if hit.payload['title'] != song_title or hit.payload['language'] != language:
                    result = MusicSearchResult(
                        file_path=hit.payload['file_path'],
                        title=hit.payload['title'],
                        language=hit.payload['language'],
