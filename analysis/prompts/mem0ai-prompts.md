...
## 1. FACT_RETRIEVAL_PROMPT（旧版，基础事实提取）
...
## 2. DEFAULT_UPDATE_MEMORY_PROMPT（旧版，记忆更新）
...
## 3. ADDITIVE_EXTRACTION_PROMPT（V3核心版本，Add-only策略）

这是 mem0ai 的核心 V3 提示词，采用 **Additive Extraction（只增不减）** 策略。
...
1. **Add-only 策略**: 只做 ADD 操作，不处理 UPDATE/DELETE
2. **记忆链接**: 通过 `linked_memory_ids` 建立记忆间关联
3. **丰富的上下文**: 提取完整的上下文信息，而非碎片化事实
4. **时间感知**: 使用 Observation Date 解析相对时间
5. **双语提取**: 同时从用户和助手消息中提取信息
