...
class AsyncPerformanceTester:
    ...
    def _generate_combinations(self):
        ...
        for llm in valid_llms:
            for tts in valid_tts:
                ...
                llm_stability = self.results["llm"][llm]["std_first_token"] / self.results["llm"][llm]["avg_first_token"]
    ...
    def _print_results(self):
        ...
        if self.results["combinations"]:
            ...
            print(tabulate(
                combo_table,
                headers=["组合方案", "综合得分", "首字耗时", "稳定性", "合成耗时"],
                tablefmt="grid",
        ...
        for llm_name, data in llm_results.items():
            if len(data["first_token_times"]) >= len(self.test_sentences) * 0.5:
                self.results["llm"][llm_name] = {
                    ...
                    "std_first_token": statistics.stdev(data["first_token_times"]) if len(data["first_token_times"]) > 1 else 0,
